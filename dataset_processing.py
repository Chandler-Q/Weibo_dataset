...
for i in range(len(news_files)):
    ...
    x = np.array([t.astype('float32'),
                  np.array(graph['edge_index']),graph['y'].values])
    
    np.save( os.path.join(raw_data_path,'Weibo','raw','bert',f'{news_files[i][:-5]}'), x)
    ...
    x = np.array([np.concatenate((t,norm.fit_transform(graph['x'].iloc[:,5:].values)),axis=-1).astype('float32'),
                  np.array(graph['edge_index']),graph['y'].values])
    np.save( os.path.join(raw_data_path,'Weibo','raw','spacy',f'{news_files[i][:-5]}'), x)
    
    # profile encoding
    x = np.array([graph['x'].iloc[:,5:].values.astype('float32'),
                  np.array(graph['edge_index']),graph['y'].values])
    
    np.save( os.path.join(raw_data_path,'Weibo','raw','profile',f'{news_files[i][:-5]}'), x)
